...
/* =========================
   RETURN BOOKS
========================= */
export const returnBorrow = async (borrowId: string) => {
  return prisma.$transaction(async (tx) => {
    ...
    return borrowRepo.updateReturn(
      borrowId,
      {
        status: 'RETURNED',
        returnedAt: now,
...
/* =========================
   MEMBER HISTORY
========================= */
export const getMyBorrowings = async (memberId: string) => {
...
/* =========================
   ADMIN: ALL BORROW
========================= */
export const getAllBorrowings = async (filter: BorrowFilter) => {
  ...
  if (filter.startDate || filter.endDate) {
    where.borrowedAt = {};
    if (filter.startDate) where.borrowedAt.gte = filter.startDate;
    if (filter.endDate) where.borrowedAt.lte = filter.endDate;
  }
